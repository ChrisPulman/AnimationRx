...
public sealed class WpfCoreTests
{
    ...
    {
        var values = await PixelsPerSecondInput
            .ToObservable()
            .PixelsPerSecond(20)
            .ToArray()
            .ToTask();
    ...
    {
        await Assert.That(() => Animations.Sequence(null!)).Throws<ArgumentNullException>();
    }

    [Test]
    public async Task ParallelRejectsNullAnimations()
    {
        await Assert.That(() => Animations.Parallel(null!)).Throws<ArgumentNullException>();
    }

    [Test]
    public async Task StaggerRejectsNullAnimations()
    {
        await Assert.That(() => Animations.Stagger(null!, TimeSpan.Zero)).Throws<ArgumentNullException>();
    }

    [Test]
    public async Task RepeatAnimationRejectsNullAnimation()
    {
        await Assert.That(() => Animations.RepeatAnimation(null!)).Throws<ArgumentNullException>();
    }
