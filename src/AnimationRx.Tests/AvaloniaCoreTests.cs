...
using RxEases = AvaloniaRx::CP.AnimationRx.Eases;
...
/// <summary>Contains Avalonia core animation tests.</summary>
public sealed class AvaloniaCoreTests
{
    ...
    /// <summary>Verifies eases facade to duration converts doubles.</summary>
    /// <returns>A task that completes when the test finishes.</returns>
    [Test]
    public async Task EasesFacadeToDurationConvertsDoubles()
    {
        var values = await ReadPercentsAsync(RxEases.ToDuration(Percentages.Take(SampleCount).ToObservable()));

        await AssertSequencesAreEqualAsync(values, Percentages.Take(SampleCount).ToArray());
    }

    /// <summary>Verifies pixels per second facade scales elapsed milliseconds.</summary>
    /// <returns>A task that completes when the test finishes.</returns>
    [Test]
    public async Task PixelsPerSecondFacadeScalesElapsedMilliseconds()
    {
    ...
    public async Task DistanceScalesDurationPercentages()
    {
        ...
        var values = await RxAnimationsExtensions.Distance(source, DistanceScale).ToArray().ToTask();
    ...
    public async Task DistanceUsesLatestObservableDistance()
    {
        ...
        var valuesTask = RxAnimationsExtensions.Distance(
            source,
            Observable.Return(LatestDistance)).ToArray().ToTask();
    ...
    {
        var source = PixelsPerSecondInput.ToObservable();
        var values = await RxAnimationsExtensions.PixelsPerSecond(source, PixelVelocity).ToArray().ToTask();
    ...
    public async Task TakeOneEveryDelaysEachValueInSequence()
    {
        ...
        using var subscription = RxAnimationsExtensions.TakeOneEvery(
    ...
    public async Task SequenceRunsAnimationsInOrder()
    {
        ...
        await RxAnimationsExtensions.Sequence(animations).ToTask();
    ...
    {
        var values = await RxAnimationsExtensions.Parallel([
    ...
    {
        var values = await RxAnimationsExtensions.RepeatAnimation(
            Observable.Return(Unit.Default),
    ...
    public async Task DelayBetweenAddsTheRequestedDelay()
    {
        ...
        using var subscription = RxAnimationsExtensions.DelayBetween(
            animations,
            TimeSpan.FromMilliseconds(StepDelayMilliseconds),
            scheduler).Subscribe();
        scheduler.Start();
    ...
    public async Task StaggerAddsIncrementalDelays()
    {
        ...
        var staggered = RxAnimationsExtensions.Stagger(
            [
                Observable.Return(Unit.Default).Do(_ => values.Add(FirstSequenceValue)),
                Observable.Return(Unit.Default).Do(_ => values.Add(SecondSequenceValue))
            ],
    ...
    {
        await Assert.That(() => RxAnimationsExtensions.Sequence(null!)).Throws<ArgumentNullException>();
    }
    ...
    {
        await Assert.That(() => RxAnimationsExtensions.Parallel(null!)).Throws<ArgumentNullException>();
    }
    ...
    {
        await Assert.That(() => RxAnimationsExtensions.Stagger(null!, TimeSpan.Zero)).Throws<ArgumentNullException>();
    }
    ...
    {
        await Assert.That(() => RxAnimationsExtensions.RepeatAnimation(null!)).Throws<ArgumentNullException>();
    }
    ...
    {
        await Assert.That(() => RxAnimationsExtensions.DelayBetween(
            null!,
            TimeSpan.Zero)).Throws<ArgumentNullException>();
    }
